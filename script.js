...
function bindHandlers(){
  ...
  voteListEl.addEventListener('click', async (e)=>{
    ...
    const nameEl = d.querySelector('#platformName');   // اگر داری
    const tagEl  = d.querySelector('#platformTagline'); // اگر داری
    ...
    try {
      ...
      if (res && res.success === false && res.code === 'disabled') { alert('فعلاً افزودن گزینه جدید غیرفعال است.'); return; }
      ...
      if (nameEl) nameEl.value=''; if (tagEl) tagEl.value='';
    } catch (error) {
